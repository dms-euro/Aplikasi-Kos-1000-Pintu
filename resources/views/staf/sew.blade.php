    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-5 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-green-100 p-5 flex items-center justify-between hover:shadow-md transition">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-800">{{ $pemesanan->filter(function($p) { return $p->pembayaran->isNotEmpty(); })->count() }}</p>
        ...
        <div class="bg-white rounded-2xl border border-amber-100 p-5 flex items-center justify-between hover:shadow-md transition">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-800">{{ $pemesanan->filter(function($p) { return $p->pembayaran->isEmpty(); })->count() }}</p>
        ...
        <!-- Link ke Riwayat -->
        <a href="{{ route('petugas.konfirmasi.history') }}" class="bg-white rounded-2xl border border-purple-100 p-5 flex items-center justify-between hover:shadow-md transition group">
            <div>
            ...
            <div class="w-12 h-12 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 text-2xl group-hover:bg-purple-200 transition">
                ...
                <tbody class="divide-y divide-indigo-100">
                    ...
                    <tr class="hover:bg-indigo-50/40 transition">
                        ...
                        <td class="px-4 py-3">
                            ...
                            @endphp
                            @if($pembayaran)
                                <span class="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs flex items-center gap-1 w-fit">
                                    <i class='bx bx-check-circle'></i> Sudah Bayar
                                </span>
                            @else
                                <span class="bg-amber-100 text-amber-700 px-2 py-1 rounded-full text-xs flex items-center gap-1 w-fit">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex gap-2">
                                <a href="{{ route('petugas.konfirmasi.show', $item->id) }}"
                                ...
                                @if($item->pembayaran->isNotEmpty())
                                    <button onclick="openConfirmModal({{ $item->id }})"
...
<div id="confirmModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl max-w-md w-full p-6">
        ...
        <form id="confirmForm" method="POST">
            @csrf
            <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-medium transition mb-2">
                Ya, Konfirmasi
            </button>
            <button type="button" onclick="closeConfirmModal()" class="w-full border border-gray-300 hover:bg-gray-50 py-3 rounded-xl font-medium transition">
                Batal
            </button>
        </form>
    </div>
    ...
    <div class="bg-white rounded-2xl max-w-md w-full p-6">
        ...
        <form id="cancelForm" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="alasan" rows="3" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-indigo-500" placeholder="Contoh: Belum melakukan pembayaran, data tidak valid, dll." required></textarea>
            </div>
            <button type="submit" class="w-full bg-rose-600 hover:bg-rose-700 text-white py-3 rounded-xl font-medium transition mb-2">
                Ya, Batalkan
            </button>
            <button type="button" onclick="closeCancelModal()" class="w-full border border-gray-300 hover:bg-gray-50 py-3 rounded-xl font-medium transition">
                Batal
            </button>
        </form>
    </div>
